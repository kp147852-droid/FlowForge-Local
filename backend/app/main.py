...
from .schemas import JobRead, RuleCreate, RuleRead, RuleToggle
...
app = FastAPI(title="Local Workflow Automator", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def api_run_rule(rule_id: int, file_path: str) -> dict[str, int]:
    rule = repository.get_rule(rule_id)
    if not rule:
        raise HTTPException(status_code=404, detail="Rule not found")
    try:
        job_id = engine.process_file_for_rule(rule, file_path)
